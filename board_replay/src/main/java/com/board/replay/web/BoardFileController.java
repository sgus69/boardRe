...
import com.board.replay.dto.board.file.BoardFileRequestDto;
import com.board.replay.dto.board.file.BoardFileRequestDto;
	...
	public void downloadFile(@RequestParam()Long id, HttpServletResponse response)throws Exception{
		try {
			...
			if(fSize > 0) {
				...
				String encoderFilename = "attachment; filename*=" + 
										"UTF-8"+"''"+ URLEncoder.encode(fileInfo.getOrigFileName(), "UTF-8");
				//ContentType 설정
				response.setContentType("application/octet-stream; charset=utf-8");
				
				//Header 설정
				response.setHeader("Content-Disposition", encoderFilename);
